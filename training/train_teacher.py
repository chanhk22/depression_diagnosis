...
def eval_epoch(model, loader, device, th=0.5):
    ...
    y_true, y_prob, y_reg = np.array(y_true), np.array(y_prob), np.array(y_reg)
    ...
    mse = mean_squared_error(y_true, y_reg)
    mae = mean_absolute_error(y_true, y_reg)
    try: pr,_ = pearsonr(y_true, y_reg)
    except: pr = float("nan")

    return {"acc":acc,"f1":f1,"precision":prec,"recall":rec,
            "auc":auc,"mse":mse,"mae":mae,"pearson":pr}
